{
  ...
  outputs = { self, nixpkgs, home-manager, lanzaboote, determinate, impermanence, nix-flatpak, nixpkgs-wayland, disko, ... } @ inputs: {
    nixosConfigurations.nixos = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        ...
        ({ config, pkgs, lib, ... }: {
            ...
            initrd = {
              ...
              systemd.services.rollback = {
                ...
                script = ''
                  ...
                  cut -f9 -d' ' | while read sub; do btrfs subvolume delete "/mnt/$sub";
                  done
          ...
          # Disko handles the main fileSystems configuration. We only need to tell NixOS
          # that the /persistent mount generated by Disko is required for boot.
          ...
          services = {
            ...
            scx = { enable = true; scheduler = "scx_bpfland"; };
            pipewire = {
              enable = true;
              alsa.enable = true;
          ...
          users.users.nix = {
            isNormalUser = true; shell = pkgs.nushell;
            description = "nix user"; extraGroups = [ "wheel" "tape" "video" "render" "seat" "audio" "input" ];
          ...
          home-manager.users.nix = { pkgs, ... }: {
            ...
            home.persistence."/persistent" = {
              directories = [ ".config" ".gnupg" ".local/share" ".steam" ".ssh" ".var/app" "Documents" "Downloads" ];
              files = [  ];
            };
            ...
            programs = {
              ...
              nushell = {
                ...
                configFile.text = ''
                  ...
                  def update [] { sudo nix flake update --flake ~/Documents/git/nixos/;
                  sudo nixos-rebuild switch --flake ~/Documents/git/nixos/ }
                  def push [msg?: string] {
                    $env.GPG_TTY = (tty);
                    gpg-connect-agent updatestartuptty /bye | ignore; git add -A
                    let m = if ($msg | is-empty) { (date now | format date '%Y-%m-%d %H:%M:%S') } else { $msg };
                    git commit -m $m; git push
                  }
                  $env.SSH_AUTH_SOCK = $"/run/user/(id -u)/gcr/ssh";
                  $env.GPG_TTY = (tty); gpg-connect-agent updatestartuptty /bye | ignore
